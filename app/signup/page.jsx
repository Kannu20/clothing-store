...
import {
  ...
  Phone, AlertCircle, ArrowRight, CheckCircle
} from "lucide-react";
...
const STRENGTH_LABELS = ["", "Weak", "Fair", "Good", "Strong"];
const STRENGTH_COLORS = ["", "bg-red-400", "bg-amber-400", "bg-blue-400", "bg-emerald-400"];
const STRENGTH_TEXT   = ["", "text-red-400", "text-amber-500", "text-blue-500", "text-emerald-500"];
...
export default function SignupPage() {
  ...
  const validate = () => {
    ...
    if (!form.email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email))
      e.email = "Enter a valid email address";
    // Phone: optional but if provided must be valid Indian number
  ...
  const handleSignup = async (ev) => {
    ...
    if (!validate()) return;
    setSubmitting(true);
    // Pass phone so it's saved to MongoDB during sync
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <span className="w-7 h-7 border-2 border-stone-200 border-t-stone-700 rounded-full animate-spin" />
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50 px-6">
        ...
        >
          ...
          {/* Phone reminder if they added one */}
          {form.phone && (
            <div className="bg-blue-50 border border-blue-100 p-4 mb-5 text-left">
              <p className="font-body text-xs text-blue-700 leading-relaxed">
                <span className="font-medium">📱 Verify your phone</span> — once you're in your account, you can verify{" "}
                <span className="font-medium">+91 {form.phone}</span> with an OTP to unlock address autofill and faster checkout.
              </p>
            </div>
          )}
          ...
          <div className="bg-amber-50 border border-amber-100 p-4 mb-7 text-left">
            <p className="font-body text-xs text-amber-700">
              <span className="font-medium">Check your inbox</span> — the email verification link expires in 24 hours. Check your spam folder too.
            </p>
          </div>
          ...
          <Link
            href="/"
            className="block font-body text-xs text-stone-400 hover:text-stone-700 underline underline-offset-2 transition-colors"
          >
            Back to Home
          </Link>
        </motion.div>
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
      ...
      {/* Left — Brand panel */}
      {/* ✅ "relative overflow-hidden" keeps the overlay inside this column */}
      <div
        className="hidden lg:block relative overflow-hidden"
        style={{
          backgroundImage:    "url(https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=1200&q=85)",
      ...
      <div className="flex flex-col justify-center px-6 py-10 sm:px-12 lg:px-16 xl:px-20 bg-white overflow-y-auto">
        <Link
          href="/"
          className="lg:hidden font-display text-2xl font-light tracking-[0.3em] uppercase text-stone-900 mb-8 block"
        >
          Luxe
        </Link>
        ...
        >
          ...
          <h1 className="font-display text-4xl font-light text-stone-900 mb-7">
            Create Account
          </h1>
          ...
          <form onSubmit={handleSignup} className="space-y-5">
            ...
            <div>
              <label className="block font-body text-[10px] tracking-[0.2em] uppercase text-stone-500 mb-2">
                Mobile Number{" "}
                <span className="text-stone-300 normal-case tracking-normal text-[9px]">(optional — for OTP verification)</span>
              </label>
              <div className="relative flex items-center">
                <Phone size={15} className="absolute left-0 top-1/2 -translate-y-1/2 text-stone-300 shrink-0" />
                {/* India prefix */}
                <span className="absolute left-6 top-1/2 -translate-y-1/2 font-body text-sm text-stone-400 select-none">
                  +91
                </span>
                <input
                  type="tel"
                  ...
                  className={`w-full border-b bg-transparent py-3 pl-14 font-body text-sm text-stone-900 placeholder:text-stone-300 focus:outline-none transition-colors ${
                    errors.phone ? "border-red-400" : "border-stone-200 focus:border-stone-700"
                  }`}
                />
              </div>
              {errors.phone && (
                <p className="font-body text-xs text-red-500 mt-1.5">{errors.phone}</p>
              )}
              <p className="font-body text-[10px] text-stone-300 mt-1.5">
                You can verify your number with OTP after signing up.
              </p>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Min. 6 characters"
                  className={`w-full border-b bg-transparent py-3 pl-6 pr-8 font-body text-sm text-stone-900 placeholder:text-stone-300 focus:outline-none transition-colors ${
                    errors.password ? "border-red-400" : "border-stone-200 focus:border-stone-700"
                  }`}
                />
                <button
                  type="button"
                    ...
                    {[1, 2, 3, 4].map((s) => (
                      <div
                        ...
                        className={`h-1 flex-1 rounded-full transition-all duration-300 ${
                          strength >= s ? STRENGTH_COLORS[strength] : "bg-stone-100"
                        }`}
                      />
                    ))}
                  </div>
                  <p className={`font-body text-[10px] ${STRENGTH_TEXT[strength]}`}>
                    {STRENGTH_LABELS[strength]} password
                  </p>
                </div>
              )}
              {errors.password && (
                <p className="font-body text-xs text-red-500 mt-1.5">{errors.password}</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Repeat your password"
                  className={`w-full border-b bg-transparent py-3 pl-6 pr-8 font-body text-sm text-stone-900 placeholder:text-stone-300 focus:outline-none transition-colors ${
                    errors.confirm ? "border-red-400" : "border-stone-200 focus:border-stone-700"
                  }`}
                />
                <button
                  type="button"
              ...
              {form.confirm && (
                <p
                  className={`font-body text-[10px] mt-1.5 ${
                    form.password === form.confirm ? "text-emerald-500" : "text-red-400"
                  }`}
                >
            ...
            <div>
              <label className="flex items-start gap-3 cursor-pointer">
                <button
                  ...
                  className={`mt-0.5 w-4 h-4 border flex items-center justify-center shrink-0 transition-colors ${
                    agreed ? "bg-stone-900 border-stone-900" : "border-stone-300"
                  }`}
                >
                  {agreed && (
                    <svg viewBox="0 0 10 8" className="w-2.5 fill-none stroke-white" strokeWidth="1.5">
                      <path d="M1 4l3 3 5-6" />
                    </svg>
                  )}
                </button>
                <span className="font-body text-xs text-stone-500 leading-relaxed">
                  I agree to the{" "}
                  <a href="#" className="text-stone-900 underline underline-offset-1">
                    Terms of Service
                  </a>{" "}
                  and{" "}
                  <a href="#" className="text-stone-900 underline underline-offset-1">
              ...
              {errors.terms && (
                <p className="font-body text-xs text-red-500 mt-1.5">{errors.terms}</p>
              )}
            </div>
...
function FormField({ icon: Icon, label, name, type, value, onChange, placeholder, error }) {
  return (
    <div>
      <label className="block font-body text-[10px] tracking-[0.2em] uppercase text-stone-500 mb-2">
        {label}
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`w-full border-b bg-transparent py-3 pl-6 font-body text-sm text-stone-900 placeholder:text-stone-300 focus:outline-none transition-colors ${
            error ? "border-red-400" : "border-stone-200 focus:border-stone-700"
          }`}
        />
      </div>
