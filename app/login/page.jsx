...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen grid grid-cols-1 lg:grid-cols-2">
      ...
      {/* ── Left — Brand panel ──────────────────────────────── */}
      {/* ✅ FIX 2: added "relative" so the absolute overlay stays INSIDE this column */}
      <div
      ...
      >
        {/* Dark overlay — stays inside because parent is relative ✅ */}
      ...
      <div className="flex flex-col justify-center px-6 py-12 sm:px-12 lg:px-16 xl:px-24 bg-white">
        ...
        <Link
          href="/"
          className="lg:hidden font-display text-2xl font-light tracking-[0.3em] uppercase text-stone-900 mb-10 block"
        >
          Luxe
        </Link>
...
function ForgotPassword({ email }) {
  ...
  if (!open) {
    return (
      <button
        ...
        className="font-body text-xs text-stone-400 hover:text-stone-700 underline underline-offset-2 transition-colors"
      >
        Forgot password?
      </button>
    );
  ...
  return (
    <motion.div
      ...
      className="w-full bg-stone-50 border border-stone-100 p-4"
    >
      {sent ? (
        <p className="font-body text-xs text-emerald-600">
          ✓ Reset link sent to {addr}
        </p>
      ) : (
        <div className="flex gap-2">
          <input
          ...
          <button
            type="button"
            onClick={handle}
            className="bg-stone-900 text-white font-body text-xs px-4 py-2 hover:bg-stone-700 transition-colors"
          >
            Send
          </button>
        </div>
