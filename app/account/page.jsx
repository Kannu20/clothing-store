...
export default function AccountPage() {
  ...
  const sendPhoneOtp = async (phoneNum) => {
    ...
    try {
      ...
      const verifier = new RecaptchaVerifier(auth, "recaptcha-container", {
        size: "invisible",
        callback: () => {},
        "expired-callback": () => { setPhoneError("reCAPTCHA expired. Please try again."); setPhoneState("entering"); },
      });
      await verifier.render();
      recaptchaRef.current = verifier;

      const prov = new PhoneAuthProvider(auth);
      const verificationId = await prov.verifyPhoneNumber(`+91${digits}`, verifier);
      phoneConfirmRef.current = verificationId;
      ...
      setPhoneState("entering");
      const m = {
        "auth/operation-not-allowed": "Phone sign-in not enabled in Firebase Console.",
        "auth/invalid-phone-number":  "Invalid phone number. Please check.",
      ...
      setPhoneError(m[err.code] || `Error: ${err.message}`);
    }
  };
  ...
  const verifyPhoneOtp = async () => {
    ...
    if (!phoneConfirmRef.current) { setPhoneOtpError("Session expired. Send OTP again."); setPhoneState("entering"); return; }

    setPhoneState("verifying");
    setPhoneOtpError("");

    try {
      const credential = PhoneAuthProvider.credential(phoneConfirmRef.current, code);
      await updatePhoneNumber(auth.currentUser, credential);
      ...
      if (data.success) {
        ...
        setPhoneOtpError("Saved to Firebase but profile update failed. Refresh.");
        setPhoneState("otp");
      }
    } catch (err) {
      ...
      const m = {
        ...
        "auth/credential-already-in-use": "This number is linked to another account.",
        "auth/requires-recent-login":     "Please sign out and sign back in first.",
      };
      setPhoneOtpError(m[err.code] || `Error: ${err.message}`);
      setPhoneState("otp");
    }
  };
  ...
  const verifyEmailOtp = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setEmailOtpError(data.error || "Incorrect OTP. Please try again.");
        setEmailState("otp");
      }
    } catch (err) {
      setEmailOtpError("Network error. Please try again.");
  ...
  // ── Shared OTP box input handler ───────────────────────────────────────────
  const makeOtpHandler = (digits, setDigits, setError, verifyFn, refs) => ({
    onChange: (idx, val) => {
      const d = val.replace(/\D/g, "").slice(-1);
      const n = [...digits]; n[idx] = d;
      setDigits(n); setError("");
      if (d && idx < 5) refs.current[idx + 1]?.focus();
      if (n.every((x) => x) && n.join("").length === 6) {
        setTimeout(() => { setDigits(n); verifyFn(); }, 120);
      }
    },
    onKeyDown: (idx, e) => {
  ...
  const phoneOtpHandlers = makeOtpHandler(phoneOtpDigits, setPhoneOtpDigits, setPhoneOtpError, verifyPhoneOtp, phoneOtpRefs);
  const emailOtpHandlers = makeOtpHandler(emailOtpDigits, setEmailOtpDigits, setEmailOtpError, verifyEmailOtp, emailOtpRefs);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.5 }} className="lg:col-span-1">
            <div className="bg-stone-900 p-8 text-center">
              ...
              {/* Phone badge */}
              {phone && (
                <div className={`flex items-center justify-center gap-1.5 px-3 py-1.5 mb-3 ${
                  phoneVerified ? "bg-emerald-900/20 border border-emerald-700/20" : "bg-stone-800 border border-stone-700"
                }`}>
              ...
              <button onClick={handleLogout} className="w-full flex items-center justify-center gap-2 border border-stone-700 text-stone-400 font-body text-xs tracking-widest uppercase py-3 hover:border-stone-500 hover:text-stone-200 transition-all">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.5, delay:0.1 }}>
              ...
              <div className="divide-y divide-stone-50 border border-stone-100">
                {quickLinks.map(({ icon: Icon, label, href, badge }) => (
                  <Link key={label} href={href} className="flex items-center justify-between px-6 py-5 hover:bg-stone-50 transition-colors group">
                    <div className="flex items-center gap-4">
                      <div className="w-9 h-9 bg-stone-100 flex items-center justify-center group-hover:bg-stone-200 transition-colors">
                        <Icon size={16} className="text-stone-600" />
                      </div>
            ...
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.5, delay:0.2 }}>
              ...
              <div className="border border-stone-100 divide-y divide-stone-50">
                ...
                {/* ── Email row with OTP verification ─── */}
                <div className="px-6 py-4">
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex-1 flex items-center justify-between gap-3 flex-wrap min-w-0">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        {emailVerified ? (
                          <span className="inline-flex items-center gap-1 bg-emerald-50 border border-emerald-100 text-emerald-600 font-body text-[9px] tracking-widest uppercase px-2 py-0.5 shrink-0">
                            <CheckCircle2 size={9} /> Verified
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1 bg-amber-50 border border-amber-100 text-amber-600 font-body text-[9px] tracking-widest uppercase px-2 py-0.5 shrink-0">
                            <AlertCircle size={9} /> Unverified
                          </span>
                        )}
                      </div>

                      {/* Only show Verify button for email accounts, not Google */}
                  ...
                  <AnimatePresence>
                        ...
                        <div className="bg-stone-50 border border-stone-100 p-5">
                          <div className="flex items-start justify-between mb-4">
                            <div>
                              ...
                              <p className="font-body text-[10px] text-stone-400 mt-0.5">Check your inbox (and spam folder)</p>
                            </div>
                            <button onClick={() => { setEmailState("idle"); setEmailOtpDigits(["","","","","",""]); }} className="text-stone-400 hover:text-stone-700">
                              <X size={14} />
                            </button>
                          </div>

                          {/* 6-box OTP input */}
                          <div className="flex gap-2 mb-4" onPaste={emailOtpHandlers.onPaste}>
                            {emailOtpDigits.map((digit, idx) => (
                              <input
                                ...
                                maxLength={1}
                                value={digit}
                                onChange={(e) => emailOtpHandlers.onChange(idx, e.target.value)}
                                onKeyDown={(e) => emailOtpHandlers.onKeyDown(idx, e)}
                                disabled={emailState === "verifying"}
                                className={`w-11 h-12 text-center font-body text-lg font-medium border-2 bg-white transition-all focus:outline-none ${
                                  emailOtpError ? "border-red-300 text-red-700" :
                                  digit ? "border-stone-900 text-stone-900" :
                          ...
                          <div className="flex items-center gap-3 flex-wrap">
                            <button
                              ...
                              disabled={emailOtpDigits.join("").length < 6 || emailState === "verifying"}
                            ...
                            >
                              {emailState === "verifying" ? (
                                ...
                                <><KeyRound size={12} /> Verify Email</>
                              )}
                            </button>

                            <button
                              onClick={sendEmailOtp}
                              disabled={emailCountdown > 0 || emailState === "verifying"}
                              className="flex items-center gap-1.5 font-body text-xs text-stone-400 hover:text-stone-700 disabled:opacity-40 disabled:cursor-not-allowed"
                            >
                              <RefreshCw size={11} />
                    ...
                    {emailState === "done" && (
                      <motion.div initial={{ opacity:0, y:-6 }} animate={{ opacity:1, y:0 }} className="mt-4 flex items-center gap-3 bg-emerald-50 border border-emerald-100 px-4 py-3">
                        <CheckCircle2 size={15} className="text-emerald-500 shrink-0" />
                        <p className="font-body text-xs text-emerald-700 font-medium">Email verified successfully! 🎉</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
                ...
                <div className="px-6 py-4">
                  <div className="flex items-center justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex-1 flex items-center justify-between gap-3 flex-wrap min-w-0">
                      ...
                      {!phoneVerified && phoneState === "idle" && (
                        <button
                          onClick={() => { setPhoneInput(phone ? phone.replace("+91", "") : ""); setPhoneState("entering"); }}
                          className="font-body text-xs tracking-widest uppercase text-stone-500 border border-stone-200 px-3 py-1.5 hover:border-stone-500 hover:text-stone-800 transition-all shrink-0"
                        >
                          <Phone size={11} className="inline mr-1.5" />
                          {phone ? "Verify Now" : "Add & Verify"}
                  ...
                  <AnimatePresence>
                      ...
                      <motion.div initial={{ height:0, opacity:0 }} animate={{ height:"auto", opacity:1 }} exit={{ height:0, opacity:0 }} transition={{ duration:0.25 }} className="overflow-hidden mt-4">
                        <div className="bg-stone-50 border border-stone-100 p-5">
                          ...
                          {(phoneState === "entering" || phoneState === "sending") && (
                            <div>
                              <p className="font-body text-xs font-medium text-stone-700 mb-3">
                                {phone ? "Verify your mobile number" : "Add & verify your mobile number"}
                              </p>
                              <div className="flex items-center gap-2">
                                <div className="relative flex-1">
                                  <Phone size={13} className="absolute left-0 top-1/2 -translate-y-1/2 text-stone-300" />
                                ...
                                <button onClick={() => setPhoneState("idle")} className="text-stone-400 hover:text-stone-700 p-1">
                                  <X size={15} />
                                </button>
                              </div>
                            ...
                            <div>
                              <div className="flex items-start justify-between mb-4">
                                <div>
                                  ...
                                  <p className="font-body text-[10px] text-stone-400 mt-0.5">Enter the 6-digit code from your SMS</p>
                                </div>
                                <button onClick={() => setPhoneState("entering")} className="font-body text-[10px] text-stone-400 hover:text-stone-700 underline">
                                  Change number
                                </button>
                              </div>

                              <div className="flex gap-2 mb-4" onPaste={phoneOtpHandlers.onPaste}>
                                {phoneOtpDigits.map((digit, idx) => (
                                  <input
                                    ...
                                    value={digit}
                                    onChange={(e) => phoneOtpHandlers.onChange(idx, e.target.value)}
                                    onKeyDown={(e) => phoneOtpHandlers.onKeyDown(idx, e)}
                                    disabled={phoneState === "verifying"}
                                    className={`w-11 h-12 text-center font-body text-lg font-medium border-2 bg-white transition-all focus:outline-none ${
                                      phoneOtpError ? "border-red-300 text-red-700" :
                                      digit ? "border-stone-900 text-stone-900" :
                                      "border-stone-200 focus:border-stone-500"
                                    } disabled:opacity-50`}
                                  />
                              ...
                              <div className="flex items-center gap-3 flex-wrap">
                                <button
                                  ...
                                  disabled={phoneOtpDigits.join("").length < 6 || phoneState === "verifying"}
                                  className="btn-primary py-2.5 px-5 text-[10px] gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
                                >
                                  {phoneState === "verifying" ? (
                                    <><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Verifying…</>
                                  ) : (
                                    <><KeyRound size={12} /> Verify OTP</>
                                  )}
                                </button>

                                <button
                                  onClick={() => sendPhoneOtp(phoneInput)}
                                  disabled={phoneCountdown > 0 || phoneState === "verifying"}
                                  className="flex items-center gap-1.5 font-body text-xs text-stone-400 hover:text-stone-700 disabled:opacity-40 disabled:cursor-not-allowed"
                                >
                                  <RefreshCw size={11} />
                                ...
                                <button onClick={() => setPhoneState("idle")} className="text-stone-400 hover:text-stone-700 p-1 ml-auto">
                                  <X size={14} />
                                </button>
                              </div>
                            </div>
                    ...
                    {phoneState === "done" && (
                      <motion.div initial={{ opacity:0, y:-6 }} animate={{ opacity:1, y:0 }} className="mt-4 flex items-center gap-3 bg-emerald-50 border border-emerald-100 px-4 py-3">
                        <CheckCircle2 size={15} className="text-emerald-500 shrink-0" />
                        <p className="font-body text-xs text-emerald-700 font-medium">Phone number verified successfully! 🎉</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </div>
            ...
            <motion.div initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.5, delay:0.3 }}>
              <h3 className="font-body text-xs tracking-[0.25em] uppercase text-stone-500 mb-4">Saved Addresses</h3>
              {dbUser?.addresses?.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {dbUser.addresses.map((addr, i) => (
                    <div key={i} className="border border-stone-100 p-5">
                      <p className="font-body text-[10px] tracking-widest uppercase text-stone-400 mb-2">{addr.label}</p>
                      <p className="font-body text-sm text-stone-800 font-medium">{addr.phone}</p>
                      <p className="font-body text-xs text-stone-500 leading-relaxed mt-1">{addr.line1}, {addr.city} — {addr.pincode}</p>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="border border-stone-100 p-8 text-center">
                  <MapPin size={32} className="text-stone-200 mx-auto mb-3" strokeWidth={1} />
                  <p className="font-body text-sm text-stone-400">No saved addresses yet.</p>
                  <p className="font-body text-xs text-stone-300 mt-1">Addresses are saved automatically when you place an order.</p>
                </div>
              )}
            </motion.div>
